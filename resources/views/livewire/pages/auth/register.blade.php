...
<div class="flex flex-col gap-8">
    <x-page-header title="Create an account" subtitle="We're excited for you to join." />

    <form wire:submit="register" class="flex flex-col gap-4">
        <div>
            <x-input-label for="prefix" :value="__('Title')" />
        ...
        <div>
            <label for="terms" class="inline-flex items-center gap-2">
                ...
                <span class="text-sm">{{ __('I agree to the terms and conditions and privacy policy.') }}</span>
            </label>
    ...
    <div class="flex justify-center gap-4">
        @if (Route::has('login'))
            <a href="{{ route('login') }}" wire:navigate>{{ __('Already have an account?') }}</a>
        @endif
    </div>
</div>