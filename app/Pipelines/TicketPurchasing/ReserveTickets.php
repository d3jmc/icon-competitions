...
class ReserveTickets
{
    ...
    public function handle(PurchaseTicketsDto $dto, Closure $next): PurchaseTicketsDto
    {
        if ($dto->amount < $dto->competition->min_tickets_per_user) {
            throw new NotEnoughTicketsRequestedException("You must select a minimum of {$dto->competition->min_tickets_per_user} tickets.");
        }

        if ($dto->competition->max_tickets_per_user > -1 && $dto->amount > $dto->competition->max_tickets_per_user) {
            throw new TooManyTicketsRequestedException("You must select less than {$dto->competition->max_tickets_per_user} tickets.");
        }

        $unclaimedTickets = $dto->competition->tickets()->unclaimed()->get();

        if ($dto->amount > $unclaimedTickets->count()) {
            throw new TooManyTicketsRequestedException('You have selected more tickets than there are available.');
        }
